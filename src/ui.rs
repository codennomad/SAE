...
fn render_main_interface(frame: &mut Frame, app: &App) {
    ...
    // Main layout: messages area, status bar, input area
...
fn render_messages(frame: &mut Frame, app: &App, area: ratatui::layout::Rect) {
    let messages_area = area.inner(Margin { horizontal: 1, vertical: 1 });
    ...
    if lines.is_empty() && app.state == AppState::Menu {
        ...
        lines.push(Line::from(vec![
            Span::styled("Commands:", Style::default().fg(Color::Rgb(0, 240, 255))),
        ]));
...
fn create_message_lines<'a>(message: &DisplayMessage, _app: &App) -> Vec<Line<'a>> {
    ...
    // Apply styling based on message state
    let style = match message.state {
        ...
        MessageState::FadingOut => {
            let elapsed = message.arrival_time.elapsed().as_millis();
            let fade_start = 10000; // Start fading after 10 seconds
